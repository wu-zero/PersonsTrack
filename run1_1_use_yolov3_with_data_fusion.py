...
log = Logger(os.path.join(log_path, 'all.log'), file_level='ERROR', terminal_flag=True, terminal_level='ERROR')


IMG_ORIGINAL_W, IMG_ORIGINAL_H = 1440, 2560
IMG_FOR_GPU_W, IMG_FOR_GPU_H = 608, 608
    ...
    while True:
        ...
        for camera_id in camera_id_list:
            ...
            if isinstance(frame_data, list) and len(frame_data) == 3 and frame_data[2] is not None:
                ...
                pool.apply_async(face_detect_and_identify, args=(camera_id, frame_i, frame, identify_result_queue))
                # 2 track 分发用于追踪的数据
                # resize
...
def track_gpu_calculate(camera_dict, batch_data_queue):
    ...
    camera_id_list = list(camera_dict.keys())
    gpu_calculate_result_dict = {camera_id: camera_dict[camera_id].track_gpu_calculate_result_queue
                                 for camera_id in camera_id_list}
    ...
    while True:
        ...
        boxes, features = _gpu_calculate(frame)
        put_data_to_queue(gpu_calculate_result_queue, [cameraID, frame_i, boxes, features], "TrackGpuCalculate: gpu_calculate_result_queue")
        if LOG_FLAG:
...
def track_cpu_calculate_thread(camera: CameraInfoAndData):
    ...
    while True:
        start = time.time()
        gpu_calculate_result = get_data_from_queue(gpu_calculate_result_queue, 0.5, "TrackCpuCalculate: gpu_calculate_result")
        # 如果没有读到数据
        if not gpu_calculate_result:
            continue
        cameraID, frame_i, boxes, features = gpu_calculate_result
        # deepsort_cpu
        track_new_id_list, track_delete_id_list, not_confirmed_detected_track, detected_track \
            = deepsort.update(boxes, features)
        # 往队列里放结果
        data = ['track_result', cameraID, frame_i, track_new_id_list, track_delete_id_list, not_confirmed_detected_track, detected_track]
...
def track_cpu_calculate(camera_dict):
    ...
    for camera_id in camera_id_list:
        track_cpu_calculate_thread_list.append(threading.Thread(target=track_cpu_calculate_thread, args=(camera_dict[camera_id],)))
...
def result_calculate_thread(camera: CameraInfoAndData, result_queue):
    ...
    match = Match()
    area_judge = AreaJudge(camera_size, area_info_list)
    ...
    def _deal_match_result(match_result):
        ...
        :return: 
        """
        for track_id, face_data in match_result:
            face_id = face_data['faceID']
            age = face_data['age']
            gender = face_data['gender']
            if face_id:
                person_manage.set_person_faceID(track_id, face_id)
            if age:
                person_manage.set_person_age(track_id, age)
            if gender:
                person_manage.set_person_gender(track_id, gender)
    
    while True:
        start = time.time()
        result = get_data_from_queue(track_and_identify_result_queue, 0.4, "ResultCalculate: track_and_identify_result_queue")
        if not result:
            continue
...
def result_send(areaID_list, result_queue):
    ...
    :return:
    """
    from docs.conf import MESSAGE_SEND_HOST, MESSAGE_SEND_PORT, MESSAGE_SEND_QNAME_FOR_TRACK, MESSAGE_SEND_QNAME_FOR_AREA_OCCUPANCY
    msg_sender_for_track = MsgSender(host=MESSAGE_SEND_HOST, port=MESSAGE_SEND_PORT, qname=MESSAGE_SEND_QNAME_FOR_TRACK)
    msg_sender_for_occupancy = MsgSender(host=MESSAGE_SEND_HOST, port=MESSAGE_SEND_PORT, qname=MESSAGE_SEND_QNAME_FOR_AREA_OCCUPANCY)
    data_fusion = AreaDataFusion(areaID_list, history_time_used=10)
    while True:
        track_result = get_data_from_queue(result_queue, 1, "ResultSend: result_queue")
        if track_result:
            # 发送track信息
            # print(track_result)
...
if __name__ == '__main__':
    ...
    cameraID_list = [105, 108, 107, 106]  # 106,107, 108, 105
    areaID_list = []
