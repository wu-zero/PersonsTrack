...
class Human:
    ...
    def get_useful_data(self, img_w, img_h, frame_original_w, frame_original_h):
        """
        Get Upper body box compared to img size (w, h)
        :param img_w:
        :param img_h:
        ...
        _RSHOULDER = CocoPart.RShoulder.value
        _LSHOULDER = CocoPart.LShoulder.value
        _RHIP = CocoPart.RHip.value
        _LHIP = CocoPart.LHip.value
        _THRESHOLD_PART_CONFIDENCE = 0.2
        parts = [part for idx, part in self.body_parts.items() if part.score > _THRESHOLD_PART_CONFIDENCE]
        part_coords = [(img_w * part.x, img_h * part.y) for part in parts if
                       part.part_idx in [0, 1, 2, 3, 4, 5, 8, 9, 11, 12, 14, 15, 16, 17]]  # [0, 1, 2, 5, 8, 11, 14, 15, 16, 17]]
        ...
        x2 = min(img_w - x, x2 - x) + x
        y2 = min(img_h - y, y2 - y) + y
        body_box_tlwh = [_round(x), _round(y), _round(x2-x), _round(y2-y)]
        if _round(x2 - x) == 0.0 or _round(y2 - y) == 0.0:
            return {}

        is_rhip, part_rhip = _include_part(parts, _RHIP)
        is_lhip, part_lhip = _include_part(parts, _LHIP)

        other_data['nose_point'] = [_round(frame_original_w * part_nose.x), _round(frame_original_h * part_nose.y)] if part_nose is not None else None
        other_data['rhip_point'] = [_round(frame_original_w * part_rhip.x), _round(frame_original_h * part_rhip.y)] if part_rhip is not None else None
        other_data['lhip_point'] = [_round(frame_original_w * part_lhip.x), _round(frame_original_h * part_lhip.y)] if part_lhip is not None else None
        other_data['rshoulder_point'] = [_round(frame_original_w * part_rshoulder.x), _round(frame_original_h * part_rshoulder.y)] if part_rshoulder is not None else None
        other_data['lshoulder_point'] = [_round(frame_original_w * part_lshoulder.x), _round(frame_original_h * part_lshoulder.y)] if part_lshoulder is not None else None

        result['body_box'] = body_box_tlwh
